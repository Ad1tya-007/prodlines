...
export async function fetchGitHubStats(
  ...
  console.log('fetchGitHubStats: Starting fetch for', { owner, repo, branch });
  ...
  console.log('fetchGitHubStats: Making GitHub API calls...');
  ...
  if (!repoResponse.ok) {
    ...
    if (repoResponse.status === 401) {
      throw new Error(`Invalid or expired GitHub token. Please check your GITHUB_TOKEN environment variable and restart the dev server. Status: ${repoResponse.status}`)
    }
    console.error(`GitHub API Error fetching repo (${repoResponse.status}):`, errorText)
    throw new Error(`Failed to fetch repo: ${repoResponse.status} ${repoResponse.statusText}`)
  }

  // Get branch info
  const branchResponse = await fetch(`${baseUrl}/repos/${owner}/${repo}/branches/${branch}`, { headers })
  if (!branchResponse.ok) {
    throw new Error(`Failed to fetch branch: ${branchResponse.statusText}`)
  }

  // Get contributors stats
  const contributorsResponse = await fetch(`${baseUrl}/repos/${owner}/${repo}/contributors`, { headers })
      ...
      if (commitDetailResponse.ok) {
        ...
        if (commitDetail.files) {
          commitDetail.files.forEach((file: any) => {
            if (file.filename && !file.filename.includes('node_modules') && !file.filename.includes('dist')) {
  ...
  const totalLOC = Array.from(contributorStats.values()).reduce((sum, stat) => sum + stat.additions, 0)
  ...
  const contributors = Array.from(contributorStats.entries())
    .map(([username, stats], index) => {
      ...
      return {
        ...
        trend: index % 3 === 0 ? 'up' : index % 3 === 1 ? 'down' : 'neutral' as const,
  ...
  console.log('fetchGitHubStats: Completed successfully', {
